...
A Tamil-first, individual public-representation application. Each applicant supplies their own details and signature, reviews the petition, downloads the PDF, and sends the prepared email from their own email app. There is no bulk-send or arbitrary-recipient path.
...
- The server validates all fields, resolves only active and verified Firestore recipients, creates the Tamil PDF, and returns the reviewed email draft. The applicant downloads the PDF, attaches it and presses Send in their own email app.
- Students may choose Save and continue later. The partial form can be restored only after the same email address completes a Firebase email-link sign-in.
...
## Firebase, App Check and email-link resume

1. In Firebase Authentication, enable Email/Password and Email link (passwordless sign-in), then add the production domain to authorized domains.
2. Create a reCAPTCHA Enterprise site key for the production domain, register it under Firebase App Check for the web app, set `VITE_FIREBASE_APP_CHECK_SITE_KEY`, and enforce App Check for Functions, Firestore and Authentication where supported. Monitor metrics before enforcement.
3. No Google OAuth credential, Gmail API or Gmail permission is used for the student petition flow.
...
Run `npm run lint`, `npm run test`, `npm run build`, `npm --prefix functions run build`, then exercise the emulator flow. Deploy with `firebase deploy`. Test the email-link resume flow with a separate Firebase test account and only unverified placeholder recipients in a separate emulator/project. The application does not claim a message was sent because the final sending occurs in the applicant's email app.
...
Before launch visually inspect generated PDFs on Android, iOS and desktop PDF viewers, including long Tamil names/addresses, multi-page output, page numbers and signature. Test keyboard focus, screen reader labels, expired email links, resume with a different email address, invalid App Check, disabled campaign, mobile widths and all legal links.
...
- Confirm App Check, Firebase ID token, input/size limits, resume-email equality and rate controls in production.
- Obtain local legal/privacy review. This repository does not claim government affiliation.
